...
btservice::btservice(openauto::configuration::IConfiguration::Pointer config)
  ...
  if (config->getAutoconnectBluetooth())
    connectToBluetooth(QBluetoothAddress(QString::fromStdString(
                           config->getLastBluetoothPair())),
                       address);
}

void btservice::connectToBluetooth(QBluetoothAddress addr,
                                   QBluetoothAddress controller) {
  // The raspberry pi has a really tough time using bluetoothctl (or really
  // anything) to connect to an Android phone even though phone connecting to
  // the pi is fine. I found a workaround where you can make the pi attempt an
  // rfcomm connection to the phone, and it connects immediately This might
  // require setting u+s on rfcomm though Other computers with more sane
  // bluetooth shouldn't have an issue using bluetoothctl
  ...
  QStringList args;
  args << "stdbuf" << "-oL" << "rfcomm" << "connect" << "hci0"
       << addr.toString() << "2";
  btConnectProcess = new QProcess();
  OPENAUTO_LOG(info)
      << "[btservice] Attempting to connect to last bluetooth device, "
      << addr.toString().toStdString() << " with `" << program.toStdString()
      << " " << args.join(" ").toStdString() << "`";
  btConnectProcess->start(program, args,
