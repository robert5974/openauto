...
QtAudioInput::QtAudioInput(uint32_t channelCount, uint32_t sampleSize,
                           uint32_t sampleRate)
    : ioDevice_(nullptr) {
...
void QtAudioInput::createAudioInput() {
  ...
  audioInput_ = (std::make_unique<QAudioSource>(
      QMediaDevices::defaultAudioInput(), audioFormat_));
}
...
void QtAudioInput::stop() { emit stopRecording(); }
